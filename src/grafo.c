...
int adicionarRota(Grafo *grafo, const char *codigoOrigem, const char *codigoDestino, int distanciaKm) {
    ...
    while (atual != NULL) {
        if (atual->destino == destino) {
            atual->voo.distanciaKm = distanciaKm;
            printf("Rota ja existia. Distancia atualizada. Numero do voo: %d.\n", atual->voo.numeroVoo);
            return 1;
        }
        atual = atual->proximo;
    }
    ...
    novaRota->voo.numeroVoo = grafo->proximoNumeroVoo;
    novaRota->voo.distanciaKm = distanciaKm;
    grafo->proximoNumeroVoo++;
    novaRota->proximo = grafo->matrizEsparsa[origem];
    grafo->matrizEsparsa[origem] = novaRota;

    printf("Voo cadastrado com numero %d.\n", novaRota->voo.numeroVoo);
    return 1;
}
...
int removerRotaPorNumero(Grafo *grafo, int numeroVoo) {
    ...
    for (i = 0; i < grafo->quantidade; i++) {
        ...
        while (atual != NULL) {
            if (atual->voo.numeroVoo == numeroVoo) {
                if (anterior == NULL) {
                    grafo->matrizEsparsa[i] = atual->proximo;
                } else {
...
void listarRotas(Grafo *grafo) {
    ...
    for (i = 0; i < grafo->quantidade; i++) {
        ...
        while (atual != NULL) {
            printf("Voo %d | %s -> %s | destino: %s | %d km\n",
                   atual->voo.numeroVoo,
                   grafo->aeroportos[i].codigo,
                   grafo->aeroportos[atual->destino].codigo,
                   grafo->aeroportos[atual->destino].cidade,
                   atual->voo.distanciaKm);
            encontrou = 1;
            atual = atual->proximo;
        }
    }
...
void listarVoosSaindoDe(Grafo *grafo, const char *codigoOrigem) {
    ...
    if (grafo == NULL || codigoOrigem == NULL) {
        return;
    }
    ...
    if (!encontrou) {
        printf("Nenhum voo saindo desse aeroporto.\n");
    }
}

static void imprimirCaminho(Grafo *grafo, int caminho[], int tamanho) {
    int i;
...
static void buscarTrajetosRecursivo(Grafo *grafo, int atualIndice, int destinoIndice,
                                    int visitado[], int caminho[], int tamanho,
                                    int *encontrou) {
    NoRota *rota;

    visitado[atualIndice] = 1;
    caminho[tamanho] = atualIndice;
    tamanho++;

    if (atualIndice == destinoIndice) {
        imprimirCaminho(grafo, caminho, tamanho);
        *encontrou = 1;
    } else {
        rota = grafo->matrizEsparsa[atualIndice];
        while (rota != NULL) {
            if (!visitado[rota->destino]) {
                buscarTrajetosRecursivo(grafo, rota->destino, destinoIndice,
                                        visitado, caminho, tamanho, encontrou);
            }
            rota = rota->proximo;
        }
    }

    visitado[atualIndice] = 0;
}
...
void listarTrajetos(Grafo *grafo, const char *codigoOrigem, const char *codigoDestino) {
    ...
    visitado = (int *) calloc(grafo->quantidade, sizeof(int));
    caminho = (int *) malloc(sizeof(int) * grafo->quantidade);

    if (visitado == NULL || caminho == NULL) {
        free(visitado);
        free(caminho);
        printf("Erro de memoria ao listar trajetos.\n");
        return;
    }

    printf("\nPossiveis trajetos de %s ate %s:\n", codigoOrigem, codigoDestino);
    buscarTrajetosRecursivo(grafo, origem, destino, visitado, caminho, 0, &encontrou);
